...
void BellmanFord();
void Dijkstra(int Nm, struct node* Nodes);
...
int getNext(struct node* Nodes, int* span, int Nm) {
	int next = 0;
	int min = LARGE;
	for (int i = 0; i < Nm; i++) {
		if (span[i] == 0 && Nodes[i].D < min) {
			next = i;
			min = Nodes[i].D;
		}
	}
...
void Dijkstra(int Nm, struct node* Nodes)
{
	...
	int cur = getNext(Nodes, span, Nm);
	while (cur != 0) {
		span[cur] = 1;
		edge = Nodes[cur].first;
		while (edge != NULL) {
			int len = edge->length;
			int end = edge->end;
			if (Nodes[end].D > len + Nodes[cur].D) {
				Nodes[end].D = len + Nodes[cur].D;
				Nodes[end].P = cur;
			}
